...
- [Android and Windows](#android-and-windows) — the same front end, off the browser, and where the APK comes from
...
Three things are tried, in order, and the first that works wins. If none of them does, the app
says so and hands you a button to **Settings → Server**; it does not guess.
...
3. **An address baked in at build time**, `EXPO_PUBLIC_AGENT_URL`. `scripts/expo.ts` sets it from
   ...
   mobile run web`, which skips the launcher and leaves the app with nothing.
...
That launcher is the local development path and nothing else, so a downloaded APK reaches step 3
with nothing in it and stops there — which is the intended answer. There used to be a fourth
step, `http://localhost:8787`, and on a phone `localhost` is the phone: a guess that fails
slowly, under a networking error that names nothing you can act on. Being told to enter an
address is better than being lied to about one.

Android and the desktop build have no origin to inherit, so they are the case that panel exists
for: open **Settings → Server**, put in the address of the machine running `npm start` —
...
Keep it on a network you trust, or put it behind something that does authenticate — a VPN back to
that network is the shape this is built for, rather than TLS on the agent.

Android additionally needs cleartext HTTP to be allowed, since a LAN server is rarely on https;
`app.json` sets `usesCleartextTraffic` for that reason, on every build profile — see
[`mobile/BUILDING.md`](mobile/BUILDING.md) for why that is not an oversight.

### Android builds

`mobile/BUILDING.md` is the whole story; the short version is that the app cannot run in Expo Go,
so an APK comes from EAS Build:

```bash
npm run mobile:apk          # eas build -p android --profile preview
```

Pushes to `main` that touch `mobile/` or `shared/` ship on their own through
`.github/workflows/android.yml`, which decides between an over-the-air update and a full binary
by fingerprinting the native runtime, and records either as a GitHub Release. `mobile/assets/` is
generated by `node mobile/scripts/gen-icons.mjs` — the mark is a terminal prompt, `>_`.
